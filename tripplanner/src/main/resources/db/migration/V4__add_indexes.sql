...
CREATE INDEX IF NOT EXISTS idx_explore_destination ON explore_items (destination);

-- explore_items.type: used in type filter
CREATE INDEX IF NOT EXISTS idx_explore_type ON explore_items (type);

-- explore_items.popularity_score: used in ORDER BY popularityScore DESC (trending)
CREATE INDEX IF NOT EXISTS idx_explore_popularity ON explore_items (popularity_score DESC);

-- shared_contents.user_id: used in findByUser_Id()
CREATE INDEX IF NOT EXISTS idx_shared_user_id ON shared_contents (user_id);

-- shared_contents.ref_id: used in findByRefId()
CREATE INDEX IF NOT EXISTS idx_shared_ref_id ON shared_contents (ref_id);

-- shared_contents.created_at: used in ORDER BY createdAt DESC
CREATE INDEX IF NOT EXISTS idx_shared_created_at ON shared_contents (created_at);

-- comments.shared_content_id: used in findBySharedContentId()
CREATE INDEX IF NOT EXISTS idx_comment_shared_content_id ON comments (shared_content_id);

-- user_votes composite: prevent duplicate votes per user per content
CREATE UNIQUE INDEX IF NOT EXISTS uk_vote_user_shared
    ON user_votes (user_id, shared_content_id);

CREATE UNIQUE INDEX IF NOT EXISTS uk_vote_user_explore
